...
Install development dependencies from the committed lockfiles:
...
Composer is required for development tests and package metadata, not for the activated plugin runtime. The WordPress distribution uses the repository-owned `autoload.php` and contains no Composer `vendor/` tree.
...
Generated build files that are intentionally tracked must be updated together with their source changes. See [docs/dependencies.md](docs/dependencies.md) for the dependency, install-script, license, host-module, PHP runtime, SBOM, audit-baseline, and major-upgrade policy.
